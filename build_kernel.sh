...
sed -i 's/missi\*|qssi\*)/missi*|qssi*|*miui*|*hyperos*|miproduct*)/g' anykernel/anykernel.sh
sed -i 's/os_string="MIUI ROM"/os_string="MIUI\/HyperOS ROM"/g' anykernel/anykernel.sh
# 调试：直接输出case片段到action日志，一眼确认是否打上
grep -A4 'case "\$userflavor"' anykernel/anykernel.sh
